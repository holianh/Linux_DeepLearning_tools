...
function TA_Tu_dong_sap_sep() {
  ...
  spreadsheet.getRange('B6:E700').activate()
...
function CheckBlank(){
  ...
  var d4 = ss.getRange("D4").getValues();
  var blk=1;
  if (b4=="") {blk=0;
...
function TA_Them_Sapxep(){
  if (CheckBlank()==1){
    ...
    var des="B" + DongCuoi + ":E" + DongCuoi;
    //Browser.msgBox(des);
    ss.getRange("A"+DongCuoi).setValue(DongCuoi-5)
    //SpreadsheetApp.getActiveSheet().getRange('F2').setValue('Hello');
    
    ss.getRange("B4:E4").copyTo(ss.getRange(des));
    ss.getRange("B4:E4").clearContent();
    TA_Tu_dong_sap_sep();
